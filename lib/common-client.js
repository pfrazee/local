...
(function (exports) {
	...
	function CommonClient__listen(container, options) {
		...
		if (options.links !== false) {
			// :TODO:MEMLEAK: CIRCULAR REFERENCE TO ELEMENT -- NEED EVENT UNBINDING
			container.addEventListener('click',     { handleEvent:CommonClient__clickHandler, container:container });
		}
		if (options.forms !== false) {
			// :TODO:MEMLEAK: CIRCULAR REFERENCE TO ELEMENT -- NEED EVENT UNBINDING
			container.addEventListener('submit',    { handleEvent:CommonClient__submitHandler, container:container });
		}
		if (options.dragdrops !== false) {
			// :TODO:MEMLEAK: CIRCULAR REFERENCE TO ELEMENT -- NEED EVENT UNBINDING
			container.addEventListener('dragstart', { handleEvent:CommonClient__dragstartHandler, container:container });
			container.addEventListener('drop',      { handleEvent:CommonClient__dropHandler, container:container });
		}
	}
	function CommonClient__unlisten(container) {
		console.log('CommonClient__unlisten not yet implemented');
		// :TODO:MEMLEAK:
	}
	...
	function CommonClient__clickHandler(e) {
		...
		// ignore if targetting _top
		if (e.target.target == '_top') { return; }
		trackFormSubmitter(e.target);
		// :NOTE: simplification -- directly uses "fromAnchor" to save time
	...
	function CommonClient__dropHandler(e) {

		// see if the user dropped onto a droptarget
		var targetElem = findParentNode.byClass(e.target, 'request-droptarget');
		if (!targetElem) { return; } // let somebody else handle the event
	...
	function CommonClient__handleResponse(targetElem, containerElem, response) {
		...
		if (response.status >= 400) {
			console.log('CommonClient: response status '+response.status, response);
			return;
		}
	}
	...
	// - :TODO: if `targetElem` is a form control, fill its value instead of replacing
	function renderResponse(targetElem, containerElem, response) {
		...
		if (response.body) {
			if (/text\/html/.test(response.headers['content-type'])) {
				...
				html = response.body.toString().replace(/</g, '&lt;').replace(/>/g, '&gt;');
			}
		}
	...
	// INTERNAL
	// subscribes all child <output>s to the form's remote endpoint's 'update' events
	// - when the update message applies to a form or for element, will issue a request for new HTML
	function subscribeOutputElements(targetElem, containerElem) {

		// find output elems
		var outputElems = targetElem.querySelectorAll('output');

		Array.prototype.forEach.call(outputElems, function(outputElem) {

			// get request config
			var form = outputElem.form || findParentNode.byTag(outputElem, 'FORM');
			var url = (form ? form.getAttribute('action') : null);
			if (!url) {
				return;
			}

			// subscribe to server's events
			var stream = Link.subscribe({ url:url });
			stream.on('update', makeUpdateEventHandler(url, outputElem, form, containerElem));
			stream.on('error', makeErrorEventHandler());
		});
	}

	function makeUpdateEventHandler(url, targetElem, formElem, containerElem) {

		var relevantNames = targetElem.getAttribute('for');
		// if we have a name, include it in the request's query
		if (targetElem.name) {
			url += '?output='+targetElem.name;
		}

		return function(m) {
			var isRelevant = false;
			// iterate the changed values
			if (Array.isArray(m.data) && relevantNames) {
				for (var i=0; i < m.data.length; i++) {
					var name = m.data[i];
					// relevant?
					if (relevantNames.indexOf(name) !== -1) {
						isRelevant = true;
						break;
					}
				}
			} else {
				// not checking individual names; just responding to the update event
				isRelevant = true;
			}
			// dispatch request if found relevant
			if (isRelevant) {
				var fieldValues = extractRequestPayload(targetElem, formElem);
				var request = { method:'get', url:url, query:fieldValues, headers:{ accept:'text/html' }};
				dispatchRequestEvent(targetElem, request);
			}
		};
	}
