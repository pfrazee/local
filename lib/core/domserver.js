if (typeof DomServer == 'undefined') {
	(function() {
		...
		DomServer.prototype.banner = function DomServer__banner() {
			var linkHeader = [
				{ methods:['post'], rel:'Agent', href:'lap://dom.env/agent', type:'application/json' },
				{ methods:['get','put','post','delete'], rel:'Node', href:'lap://dom.env/agent/{agent}/node?{selector}&{selectorAll}&{attr}&{append}&{before}&{replace}&{add}&{remove}&{toggle}', type:'text/html' },
				{ methods:['listen', 'unlisten', 'trigger'], rel:'Event', href:'lap://dom.env/agent/{agent}/event/{event}?{selector}' }
			];
		...
		DomServer.prototype.makeagent = function DomServer__makeagent(request, match) {
			...
			var agent = Env.makeAgent(params.id);
		...
		DomServer.prototype.put = function DomServer__put(request, match) {
			...
			nodes.forEach(function(node) {
				node[attr] = val;
				if (attr != 'innerHTML' && node.getAttribute(attr) != val) {
		...
		DomServer.prototype.ins = function DomServer__ins(request, match) {
			...
			nodes.forEach(function(node) {
				if (!attr || /html/i.test(attr)) {
					...
					var frag = range.createContextualFragment(val);
					if (!frag.firstChild.tagName) {
						// this known to occur if a <tr> was given
						frag = range.createContextualFragment('<table>'+val+'</table>');
						frag = frag.querySelector('tr');
					}
					...
					if (request.query.toggle == '1') {
						node.classList.toggle(val);
					} else if (request.query.remove == '1') {
