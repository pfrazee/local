...
if (typeof Http == 'undefined') {
	(function() {
					...
					for (var j=0; j < server.inst.routes.length; j++) {
						...
						for (var k in route.match) {
							...
							if (route.match[k] instanceof RegExp) {
								...
								if (!match) {
									Util.log('routing', ' > ',/*server.inst,*/route.cb,'MISS ('+k+' "'+reqVal+'" vs '+route.match[k]+')');
									break;
								}
								matches[k] = match;
							}
		...
		Router.prototype.dispatch = function Router__dispatch(request, opt_cb, opt_context) {
			...
			Util.log('traffic', '|>', (request.org) ? request.org+':' : '', request.__mid, request.method, request.uri, request);

			// URIs that dont target hash URIs should be fetched remotely
			var protocol = request.uri.split('://');
			if (protocol.length == 2 && protocol[0] != 'lap') {
		...
		function Router__finishHandling(request, response) {
			...
			Util.log('traffic', ' >|', (request.org) ? request.org+':' : '', request.__mid, response.code, response.reason, response);
		...
		// Link Reflector
		// ==============
		function reflectLinks(links, static_params, dispatch_fn) {
			if (!links) { return null; }
			if (!Array.isArray(links)) { links = [links]; }
			dispatch_fn = dispatch_fn || Agent.dispatch;
			var fns = {};
			links.forEach(function(link) {
				var methods = Array.isArray(link.methods) ? link.methods : [link.method];
				if (methods.length === 0) { methods = ['get']; }
				methods.forEach(function(method) {
					var fn_name = method + Util.ucfirst(link.rel);
					fn_name = fn_name.replace(/ /g, '_');
					fns[fn_name] = reflectLinks__makeFunc(method, link, static_params, dispatch_fn);
				});
			});
			return fns;
		}
		// moved here to reduce closure size
		function reflectLinks__makeFunc(method, link, static_params, dispatch_fn) {
			return function(params, opt_body, opt_type, opt_headers) {
				var k;
