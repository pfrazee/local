define(['./spahql-min'], function() {
    var Module = function() {
    };

    Module.prototype.resources = {
        '/': {
            desc:'Dan Glegg\'s SpahQL: Query, manipulate and manage JSON data effortlessly.',
            _get:'Hello banner.',
            _post:{
                desc:'Applies select, or replace if a replace header is provided'
            }
        }
    };

    Module.prototype.routes = [
        { cb:"banner", uri:"^/?$", method:'get' },
        { cb:"apply", uri:"^/?$", method:'post' }
    ];
    ...
    Module.prototype.apply = function(request) {
        ...
        var data = {};
        if (request.body) { data = Link.getTypeInterface(request['content-type'], request.body).getData(); }
