...
require(paths, function(_, IncEnv, LinkshuiCli, LinkshuiHistory, LinkshuiOrderDm) {
    // Enable proxy
    ...
    env.addModule('#dm', new LinkshuiOrderDm({ uri:'#dm', container_id:'lshui-env' }));
    ...
    for (var i=0; i < env_config.structure.length; i++) {
        ...
        env.addModule(uri, new Module(env_config.structure[i]));
    }
