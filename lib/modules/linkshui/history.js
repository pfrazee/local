define(function() {
    ...
    var RootResource = {
        ...
        GET:'Provides an array of the stored entries.',
        POST:{
            desc:'Adds an entry to the history.',
            asserts:function(request) {
                if (!request.body) { throw { code:400, "Body of type js/object is required." }; }
                if (typeof request.body != 'object') { throw { code:400, "Body of type js/object is required." }; }
                if (!request.body.cmd) { throw { code:400, "`cmd` key must specify the command used to invoke the entry." }; }
                if (!request.body.response) { throw { code:400, "`response` key must specify the final response to the command." }; }
            }
        }
    };
    var EntryResource = {
        desc:'History entry.',
        GET:'Provides the history entry.'
    };
    // initial resources
