...
define(function() {
        ...
        '/':{
            ...
            _post:{
                ...
                asserts:function(request) {
                    ...
                    if (!body) { throw { code:400, reason:"Body is required." }; }
                    if (!body.cmd) { throw { code:400, reason:"`cmd` key must specify the requests the CLI should run." }; }
                }
            }
        }
    };
    ...
    CLI.prototype.commandHandler = function(request) {
        ...
        var dispatch = function(req, handler) {
            // If not given a pound-sign, make the URI relative to the current hash
            if (req.uri.charAt(0) != '#') {
                if (window.location.hash) {
                    req.uri = window.location.hash + '/' + req.uri;
                } else {
                    req.uri = '#' + req.uri;
                }
            }
