...
define(['link'], function() {
    ...
    CLI.prototype.resources = {
        GET:'Linkshui CLI parser & executor.',
        POST:'Parses the given command into a request chain, then executes the chain by piping each response into the next request.'
    };

    // Route handlers
    // ==============
    CLI.prototype.routes = {
        commandHandler:{ uri:'^/?$', method:'post' }
    };
    CLI.prototype.commandHandler = function(request, match, structure) {
        ...
        var dispatch = function(req, handler) {
            ...
            structure.dispatch(req, function(res) {
        ...
        var handleResponse = function(res) {
            // If the user specified a type, convert it now
            if (res.body && cur_request.accept && cur_request.accept != res['content-type']) {
                var type_iface = Link.getTypeInterface(res['content-type'], res.body);
                res.body = type_iface.convertToType(cur_request.accept);
                if (!res.body) { res = { code:415, reason:'media type not supported' }; }
                else { res['content-type'] = cur_request.accept; }
            }
            ...
            if (res.code >= 400 || res.code == 0) {
                ...
                // Send to history
                structure.dispatch({ uri:'#hist', method:'post', 'content-type':'js/object', body:{ cmd:body.cmd, response:res }}, function() {
