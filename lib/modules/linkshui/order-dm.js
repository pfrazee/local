define(['link'], function() {
    ...
    OrderDm.prototype.resources = {
        GET:'Linkshui div manager. Used to provide controlled access to sections of the browser DOM.',
        POST:'Allocates a new div and begins routing requests to the given control uri.'
    };
    var DivResource = {
        GET:'Active div interface. If a control uri is specific, will pipe the request to it. Otherwise, gives the div innerHTML',
        PUT:'Replaces the HTML in the div.',
        DELETE:'Removes the div from the DOM and frees its index.'
    };
    ...
    OrderDm.prototype.routes = {
        infoHandler:{ uri:'^/?$', method:'get' },
        createHandler:{ uri:'^/?$', method:'post' },
        getHandler:{ uri:'^/([0-9]+)/?$', method:'get' },
        setHandler:{ uri:'^/([0-9]+)/?$', method:'put' },
        collapseHandler:{ uri:'^/([0-9]+)/collapse/?$', method:'post' },
        deleteHandler:{ uri:'^/([0-9]+)/?$', method:'delete' },
    };
    ...
    OrderDm.prototype.createHandler = function(request, match, structure) {
        // find an open div
        for (var i=1; i < 20; i++) {
        ...
        // create
        __createDivFromRequest.call(this, i, request, structure);
        this.resources[i] = DivResource;
        // respond
        return { code:200, body:this.divs[i], 'content-type':'js/lshui.orderdm.index+object' };
    };
    OrderDm.prototype.getHandler = function(request, match, structure) {
        // validate
        ...
        if (div.ctrl_uri) {
            ...
            structure.get({ uri:div.ctrl_uri, accept:request.accept }, function(response) {
    ...
    OrderDm.prototype.setHandler = function(request, match, structure) {
        // validate
        var index = +(match.uri[1]);
        ...
        // create
        __createDivFromRequest.call(this, index, request, old_div, structure);
        return { code:200 };
    };
    OrderDm.prototype.collapseHandler = function(request, match, structure) {
        // validate
        var index = +(match.uri[1]);
        if (!(index in this.divs) || !this.divs[index]) { return { code:404, reason:'div not found' }; }
        var div = this.divs[index];
        ...
        if (div.ctrl_uri) {
            structure.post({ uri:div.ctrl_uri + '/collapse' });
        }
        return { code:205 };
    };
    OrderDm.prototype.deleteHandler = function(request, match, structure) {
        // validate
        var index = +(match.uri[1]);
        if (index < 0 || index >= 20) { return { code:400, reason:'bad index: '+index, body:'Index must be between 0 and 19.' }; }
        // notify the control
        var div = this.divs[index];
        if (div.ctrl_uri) {
            structure.dispatch({ method:'delete', uri:div.ctrl_uri });
        }
        // remove the div
        __removeDivFromDom.call(this, index);
    ...
    var __createDivFromRequest = function(index, request, old_div, structure) {
