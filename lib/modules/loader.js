...
require(paths, function(_, LinkshuiCli, LinkshuiHistory, LinkshuiOrderDm) {
    ...
    Link.attachToWindow(env, function(request, response) {
        // Add to the history
        var cmd = (request.uri == '#cli' && request.method == 'post') ? request.body.cmd : (request.method + ' ' + request.uri);
    ...
    var setPrompt = function() {
        ...
        // pick your poison
        var tickanim = ['&#9777;','&#9778;','&#9780;','&#9782;','&#9783;','&#9779;'];
        //var tickanim = []; for (var i=10240; i <= 10495; i++) { tickanim.push('&#'+i+';'); }
    ...
    setInterval(setPrompt, 1000); // Update every 10 seconds, which is precise enough for minutes
    setPrompt();
});