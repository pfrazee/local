...
(function () {
	...
	function execCallback(parentPromise, targetPromise, fn) {
		if (fn === null) {
			...
			catch (e) { console.log("Promise exception thrown", e); targetPromise.reject(e); }
	...
	function promise(v) {
		if (arguments.length > 1) {
			...
			var results = []; results.length = nArgs;
			var addResult = function(v, index) {
				results[index] = v;
				if ((++nFinished) == nArgs)
					p.fulfill(results);
			};
			for (var i=0; i < nArgs; i++)
				promise(arguments[i]).succeed(addResult, i).fail(addResult, i);
			return p;
		}
