	...
	function execCallback(parentPromise, targetPromise, fn) {
		if (fn === null) {
			...
			try { newValue = fn(parentPromise.value); }
				...
				var asStr = e.toString();
				if (asStr == '[object Object]' || asStr == e) asStr = '';
				else asStr = '("'+asStr+'")';
				if (console.warn) console.warn("Promise exception thrown", e, asStr);
				else console.log("Promise exception thrown", e, asStr);
