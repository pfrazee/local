...
local.http.ext.Navigator = Navigator;
...
Navigator.prototype.resolve = function Navigator__resolve(options) {
	...
	if (this.links !== null && (this.context.isResolved() || (this.context.isAbsolute() && this.context.isBad() === false)))
		...
		if (this.parentNavigator)
			...
			this.head(null, null, null, { noresolve:true }) // head request to our absolute url to confirm it
				.succeed(function(res) { return self.context.getUrl(); })
				.chain(resolvePromise);
	} else
...
// builder fn
local.http.ext.navigator = function(url) {
...
local.http.ext.Responder = Responder;
...
Responder.prototype.pipe = function(response, headersCB, bodyCb) {
	...
	promise
		.succeed(function(response) {
			...
			if (response.on && response.isConnOpen) {
				// wire up the stream
...
// builder fn
local.http.ext.responder = function(res) {
...
// builder fn
local.http.ext.router = function(request) {
...
local.client.handleResponse = LocalClient__handleResponse;// Regions
// =======
