...
var Environment = {};(function(exports) {
	...
	WorkerServer.prototype.onWorkerReady = function(message) {
		...
		this.worker.importScripts('/lib/worker_httpl.js');
	...
	function bindEventHandlers() {
		...
		this.element.addEventListener('request', function(e) {
			...
			if (!urld.protocol) {
				...
				var newUrl = (self.lastRequestLocation + request.url);
				// reduce the string's '..' relatives
				// :TODO: I'm sure there's a better algorithm for this
				var lastRequestHost = Link.parseUri(self.lastRequestLocation).host;
				do {
					request.url = newUrl;
