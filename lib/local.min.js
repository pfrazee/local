...
(function () {
	...
	Promise.prototype.cancel = function() {
		...
		var i;
		for (i=0; i < this.fulfillCBs.length; i++) {
			this.fulfillCBs[i].p.cancel();
		}
		for (i=0; i < this.exceptCBs.length; i++) {
			this.exceptCBs[i].p.cancel();
		}
		// free up memory
		this.fulfillCBs.length = 0;
		this.exceptCBs.length = 0;
	...
	//   - if any "except," the containing promise will except as well
	function promise(v) {
		if (arguments.length > 1) {
			var p = promise();
			var nArgs = arguments.length, nFinished = 0;
			var values = []; values.length = nArgs;
			var addValue = function(v, index) {
				values[index] = v;
				if ((++nFinished) == nArgs)
					p.fulfill(values);
			};
			for (var i=0; i < nArgs; i++)
				promise(arguments[i]).then(addValue, i).except(addValue, i);
			return p;
		}
		return (v instanceof Promise) ? v : new Promise(v);
	}
...
(function (exports) {
	...
	function CommonClient__unlisten(container) {
		...
		var outputElems = container.querySelectorAll('output, form');
		Array.prototype.forEach.call(outputElems, function(outputElem) {
			if (outputElem.__subscriptions) {
				for (var url in outputElem.__subscriptions) {
					outputElem.__subscriptions[url].close();
				}
				delete outputElem.__subscriptions;
			}
		});
	}
	...
	function subscribeOutputElements(targetElem, containerElem) {
		...
		Array.prototype.forEach.call(outputElems, function(outputElem) {
			...
			outputElem.__subscriptions = outputElem.__subscriptions || {};
			var stream = outputElem.__subscriptions[url];
			if (!stream)
				stream = outputElem.__subscriptions[url] = Link.subscribe({ url:url });
