        ...
        // local request
            ...
            req.on('end', function() {
                ...
                mediator.dispatch(link_req, function(link_res) {
                    ...
                    var body_iface = (link_res.body ? Link.getTypeInterface(link_res['content-type'], link_res.body) : null);                    
                    delete link_res.body;
                    // use the interface to produce the response
                    var body = '';
                    if (body_iface) {
                        // try to obey the accept header
                        var converted_body = body_iface.convertToType(link_req.accept);
                        if (converted_body) {
                            body = converted_body;
                            link_res['content-type'] = link_req.accept;
                        } else {
                            // just give what we have
                            body = body_iface.getData();
                        }
                    }
                    // write response
