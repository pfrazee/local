var fs = require('fs');
var path = require('path');
var Tint = require('tintjs');
var Link = require('linkjs');

// modules
var LayoutModule = require('./modules/server/layout.js').Module;
var StaticFileModule = require('./modules/server/static_file.js').Module;
var LinkshuiEnvModule = require('./modules/server/env.js').Module;

var TEMPLATE_PATH = './lib/templates';
var LayoutTemplate = null; // loaded at createServer


exports.createServer = function(config) {
    // create link app
    var structure = new Link.Structure();
    structure.addModule('#public', new LinkshuiEnvModule('./usercfg', TEMPLATE_PATH));
    structure.addModule('#public/modules', new StaticFileModule('./lib/modules'));
    structure.addModule('#public/assets', new StaticFileModule('./lib/assets'));
    
    // load the template, if needed
    if (!LayoutTemplate) { __loadLayoutTemplate(); }
    
    // create http server
    return require('http').createServer(function(req, res) {
        // build link request
        var link_req = req.headers;
        link_req.uri = '#public'+req.url;
        link_req.method = req.method;
        link_req.body = '';
        req.setEncoding('utf8');
        // collect request body
        req.on('data', function(chunk) {
            link_req.body += chunk;
        });
        req.on('end', function() {
            // run through link
            structure.dispatch(link_req, function(link_res) {
                // extract non-headers
                var code = link_res.code; delete link_res.code;
                var reason = link_res.reason || ''; delete link_res.reason;
                var body = link_res.body; delete link_res.body;
                // use the interface to produce the response
                if (body && link_req.accept != '*/*') {
                    // use a type interface to convert
                    var converted_body = null;
                    var body_iface = Link.getTypeInterface(link_res['content-type'], body);
                    // iterate through the accept types
                    var accept_types = link_req.accept.split(',');
                    for (var i=0; i < accept_types.length; i++) {
                        var type = accept_types[i];
                        converted_body = body_iface.convertToType(type);
                        if (converted_body) {
                            body = converted_body;
                            link_res['content-type'] = type;
                            break;
                        }
                    }
                }
                // make sure we give a string or buffer
                if (body && typeof body != 'string' && Buffer.isBuffer(body) != true) {
                    body = body.toString();
                }
                // write response
                res.writeHead(code, reason, link_res);
                res.end(body);
            }, null, [__layout]);
        });
    });
};

var __loadLayoutTemplate = function() {
    fs.readFile(path.resolve(path.join(TEMPLATE_PATH, 'layout.html')), 'utf8', function(err, data) {
        if (err) { console.log('Unable to find layout template at', template_path); }
        else {                
            LayoutTemplate = new Tint.compile(data,  function(title, body) {
                this.title = title;
                this.body = body;
            });
        }
    });
};

var __layout = function(handler, request, match, structure) {
    var promise = new Link.Promise();
    Link.Promise.when(handler(), function(response) {
        if (request.accept.indexOf('html') != -1) {
            // Convert to html
            var iface = Link.getTypeInterface(response['content-type'], response.body);
            if (request.accept.indexOf('html') == -1 || !iface.toHtml) { return response; }
            var html = iface.toHtml();
            
            // Embed in layout and output HTML
            var layoutTmpl = new LayoutTemplate('LinkShUI', html);
            response.body = layoutTmpl.toString();
            response['content-type'] = 'text/html';
        }
        promise.fulfill(response);
    });
    return promise;
};